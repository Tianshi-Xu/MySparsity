...
class PretrainCirNasMobileNetV2(nn.Module):
    
    def __init__(self, n_class=1000, input_size=224, width_mult=1.0):
        super(PretrainCirNasMobileNetV2, self).__init__()
        block = CirNasInvertedResidual
        input_channel = 32
        last_channel = 1280
        self.feature_size = 32
    ...
    def __str__(self):
        ...
        return super(PretrainCirNasMobileNetV2, self).__str__() + "\n" + additional_info
    
    
def cir_nas_mobilenet_cifar(n_class, input_size, width_mult) -> CirNasMobileNetV2:
    model = CirNasMobileNetV2(n_class=n_class, input_size=input_size, width_mult=width_mult)
    return model

def pretrain_cir_nas_mobilenet_cifar(n_class, input_size, width_mult) -> PretrainCirNasMobileNetV2:
    model = PretrainCirNasMobileNetV2(n_class=n_class, input_size=input_size, width_mult=width_mult)
    return model