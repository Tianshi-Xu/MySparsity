...
class CirNasMobileNetV2(nn.Module):
    def __init__(self, n_class=1000, input_size=224, width_mult=1.0,pretrain=False,finetune=False,fix_block_size=-1):
        ...
        for t, c, n, s in interverted_residual_setting:
            ...
            for i in range(n):
                if i == 0:
                    self.features.append(block(input_channel, output_channel, s, expand_ratio=t,feature_size=self.feature_size,pretrain=self.pretrain,finetune=self.finetune,block_size=fix_block_size))
                else:
                    self.features.append(block(input_channel, output_channel, 1, expand_ratio=t,feature_size=self.feature_size,pretrain=self.pretrain,finetune=self.finetune,block_size=fix_block_size))
...
def cir_nas_mobilenet(n_class, input_size, width_mult,pretrain,finetune,block_size=-1) -> CirNasMobileNetV2:
    model = CirNasMobileNetV2(n_class=n_class, input_size=input_size, width_mult=width_mult,pretrain=pretrain,finetune=finetune,fix_block_size=block_size)
    return model